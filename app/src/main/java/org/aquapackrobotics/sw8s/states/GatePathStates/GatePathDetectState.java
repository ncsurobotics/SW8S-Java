...
public class GatePathDetectState extends State {
    ...
    public boolean gateAlign() {
        return gateSubAlign(gateNoPole) || gateSubAlign(gateOnlyPole);
    }

    public boolean gateSubAlign(GatePoles gate) {
        ...
        try {
            if (gate.detected()) {
                ...
                PrintWriter printWriter = new PrintWriter(DirGate.toString() + "/" + Instant.now().toString() + ".txt");
                ...
                Imgcodecs.imwrite(DirGate.toString() + "/failure/" + Instant.now().toString() + ".jpeg", yoloout);
                return false;
            }
        } catch (Exception e) {
            e.printStackTrace();
